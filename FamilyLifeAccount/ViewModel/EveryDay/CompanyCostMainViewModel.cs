...
{
    public class CompanyCostMainViewModel : PaginViewModel
    {
        DALBase dal = new DALBase();

        public CompanyCostMainViewModel()
        {
            if (!IsInDesignMode)
            {
                InitLoad();
        ...
        {
            CompanyEditCostMain uc = new CompanyEditCostMain();
            PopUpWindow pop = new PopUpWindow(uc);
            pop.Closed += (s, e) => ViewModelLocator.ClearCompanyEditCostMain();
            if (!string.IsNullOrEmpty(id))
            {
                ...
                Messenger.Default.Send<NotificationMessage<string>, CompanyEditCostMain>(msg2);
            }
            else
            {
