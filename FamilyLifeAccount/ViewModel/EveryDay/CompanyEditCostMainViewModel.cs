...
namespace FamilyLifeAccount.ViewModel.EveryDay
{
    ...
    /// </para>
    /// </summary>
    public class CompanyEditCostMainViewModel : ViewModelBase
    {
        #region 数据初始化
        ...
        public CompanyEditCostMainViewModel()
        {
            if (!IsInDesignMode)
            {
                InitLoad();
            ...
            if (uibase.MessageShowError(MyCost.CostClassID, "支出分类"))
            {
                if (uibase.MessageShowError(MyCost.CostName, "项目"))
                {
                    try
                    {
                        ...
                        Messenger.Default.Send<NotificationMessage, CompanyCostMainViewModel>(msg);
                    }
                    catch (Exception ex)
                    {
